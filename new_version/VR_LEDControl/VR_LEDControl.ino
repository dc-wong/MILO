/*
    VR_LEDControl.ino
        Simplified version without digital writes. Controls servo based on voice commands.
...
#include "CybLicense_1712100337.h"
#define DSPOTTER_LICENSE g_lpdwLicense

// The DSpotter Keyword Model Data.
#include "Model_1712100337.h"
#define DSPOTTER_MODEL g_lpdwModel

// Define Command IDs
#define GO_UP       10000
#define GO_DOWN     10001
// You can keep other command IDs if needed or remove them if not used

Servo myservo;

// The VR engine object. Only one instance is allowed.
static DSpotterSDKHL g_oDSpotterSDKHL;

// Callback function for VR engine
void VRCallback(int nFlag, int nID, int nScore, int nSG, int nEnergy){
    Serial.print("Callback Triggered - Flag: ");
    Serial.println(nFlag);
    
    if (nFlag == DSpotterSDKHL::InitSuccess) {
        Serial.println("DSpotter SDK Initialization Successful.");
    }
    else if (nFlag == DSpotterSDKHL::GetResult) {
        Serial.print("Recognition Result - ID: ");
        Serial.print(nID);
        Serial.print(", Score: ");
        ...
        switch(nID) {
            case GO_UP:
                Serial.println("Executing GO_UP command.");
                LED_RGB_Green(); // Optional: Keep if you want to use LED indicators
                myservo.write(180); // Move servo to 180 degrees
                Serial.println("Servo moved to 180 degrees");
                break;
            case GO_DOWN:
                Serial.println("Executing GO_DOWN command.");
                LED_RGB_Red(); // Optional: Keep if you want to use LED indicators
        ...
        switch(nID) {
            case DSpotterSDKHL::TriggerStage:
                Serial.println("Entered Trigger Stage.");
                LED_RGB_Off(); // Optional: Keep if you want to use LED indicators
                LED_BUILTIN_Off(); // Optional: Keep if you want to use built-in LED
                break;
            case DSpotterSDKHL::CommandStage:
                Serial.println("Entered Command Stage.");
                LED_BUILTIN_On(); // Optional: Keep if you want to use built-in LED
                break;
            default:
                Serial.println("Unknown Stage Change.");
                break;
        }
    }
    else if (nFlag == DSpotterSDKHL::GetError) {
        Serial.print("DSpotter SDK Error - Code: ");
        Serial.println(nID);
        ...
        Serial.println("Lost Record Frame.");
...
void setup()
{
    ...
    // Initialize Servo
    myservo.attach(11); // Ensure pin 11 is correctly connected
    Serial.println("Servo attached to pin 11.");

    // Initialize LED control
    LED_Init_All(); // Optional: Keep if you want to use LED indicators
    // Removed pinMode configurations for digital writes as per request

    // Initialize PDM audio
    if (!PDM.begin(1, 16000)) {  // Mono, 16 kHz sample rate
        Serial.println("PDM Audio init failed.");
        while (1);  // Stop execution if PDM initialization fails
    } else {
        Serial.println("PDM Audio initialized successfully.");
    }

    // Enable DSpotter SDK debug info
    DSpotterSDKHL::ShowDebugInfo(true);

    // Initialize DSpotter SDK
    int initStatus = g_oDSpotterSDKHL.Init(DSPOTTER_LICENSE, sizeof(DSPOTTER_LICENSE), DSPOTTER_MODEL, VRCallback);

    if (initStatus == DSpotterSDKHL::Success) {
        Serial.println("Voice recognition initialization successful!");
    } else {
        Serial.print("Voice recognition initialization failed with error code: ");
...
// #define GO_DOWN    10001
// #define CLOSER   10010
// #define FARTHER    10011
// #define RESET 10030
// #define set_pos_one 10020
// #define set_pos_two 10021
// #define move_pos_one 10022
// #define move_pos_two 10023
...
//     Serial.print("Voice recognition initialization failed with error code: ");
//     Serial.println(initStatus);
//     // Optional: handle the failure, for example by entering a loop to prevent further execution
//     while (true) {} // Infinite loop to halt program if initialization fails
//   }
